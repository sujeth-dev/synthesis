...
**Set status:** Sets 1-2 done (2026-08-12); Sets 3-6 pending. Set 1 added and validated `topic`/`topic_order`, split Phase 1 into 5-node Computer Basics plus 14-node Programming Basics with independent Bloom evaluation sets, authored `p2_data_cleaning_preprocessing`, and restored the two-MCQ-per-guided-tier contract for the checkpoint banks. Set 2 completed all 16 Phase 4 nodes across five topics, including four plain-language explanation depths, six tiered MCQs per node, practice ladders where useful, an acyclic dependency graph, and an 18-question Bloom evaluation set enforced by the runtime and validator maps.
