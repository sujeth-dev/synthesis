...
**Status:** done · **Depends on:** BLOOM-6, Phase 1 done
...
- ~~**Full curriculum total: 119 nodes** (47 in Phase 1-3 + the OOP node, 72 in Phase 4-8), up from 53 today.~~ **Corrected 2026-08-13: the total is 120 nodes, not 119 — the original figure never added the OOP node into the Phase 1-3 subtotal (it said "47 + the OOP node" but the "47" itself already should have included it: 45+2 calc/data-cleaning+1 OOP=48, not 47).** Verified directly against the graph after Sets 1-5: 5+14+17+12+16+16+16+14 = 111 nodes so far, +9 remaining Phase 8 nodes (1 of 10 already existed as a stub) = 120. Human decision 2026-08-12 (plan doc E6): full scope is committed, **executed as 6 sequential sets**, not one pass — Set 1 (schema field + OOP/calculus/data-cleaning prerequisite nodes + Phase 1 split), Set 2 (Phase 4), Set 3 (Phase 5, after Set 1's prerequisites exist), Set 4 (Phase 6), Set 5 (Phase 7), Set 6 (Phase 8 + full-curriculum validate re-check). Each set ships its own phase evaluation file and is independently reviewable, matching how BLOOM-4 was built once against p1 then reused for p2/p3.

**Set status:** Sets 1-6 done (2026-08-13). Set 1 completed the schema, Phase 1 split, and prerequisite additions. Sets 2-4 completed all 16 nodes in each of Phases 4, 5, and 6; Set 5 completed all 14 Phase 7 nodes; Set 6 completed all 10 Phase 8 nodes and the full 120-node audit. Every completed phase includes four plain-language explanation depths, six tiered MCQs per node, practice ladders where useful, acyclic dependency edges, and an enforced 18-question Bloom evaluation. Set 3 retained the approved backpropagation checkpoint unchanged.
