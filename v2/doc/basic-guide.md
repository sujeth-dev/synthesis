...
**Live upgrade trigger (P1-5):** the rule-based classifier remains primary until NLP-1 through NLP-3 evaluate it and the versioned API grader against the same 50–200 human-labeled explanations, and the API grader demonstrates a reliable improvement in agreement with those labels. If that comparison does not improve on the rules, there is no live upgrade. If it does, the API grader may become the primary grader only after the fallback, prompt-versioning, logging, timeout, and cost controls below are ready; `classifyExplanation()` remains the hard fallback rather than being deleted. Promoting the external grader into live BKT evidence is a production-behavior cutover and therefore still requires human sign-off under `DEVELOPMENT_LOOP.md`.
