...
**Total: 8 existing + 64 new = 72 nodes across 21 main topics for Phase 4-8.** Combined with Phase 2's 2 new prerequisite-gap nodes from A6, Phase 1-3 becomes 47, and ~~the full curriculum lands at **119 nodes**~~ **corrected 2026-08-13 (found while Set 6 was blocked authoring the actual content): the full curriculum is 120 nodes, not 119.** The original "47" only ever added Phase 2's 2 new nodes to the original 45 (45+2=47) — it never added the Phase 1(b) OOP node (Section A4) into that running total, even though A4 had already flagged OOP as required before this section's total was written. Correct arithmetic: 45 (original) + 2 (calculus, data cleaning) + 1 (OOP) = 48 in Phase 1-3, + 72 in Phase 4-8 = **120**. Verified directly against the authored graph after Sets 1-5 landed: 5 (Computer Basics) + 14 (Programming Basics, 13 original + 1 OOP) + 17 (CS/Data) + 12 (Intro AI) + 16 + 16 + 16 + 14 (Phases 4-7) = 111, +9 remaining Phase 8 nodes (1 of the 10 already existed as a stub) = 120. All 10 named Phase 8 nodes in the table above are retained — none are cut to force-fit the wrong total. Deliberately not a round number — each phase's count reflects what the subject matter needs, not a padding target. Depth allocation: Phase 4/5/6 at 16 each (matching Phase 1's 18 / Phase 2's revised 17 rigor), Phase 7 at 14 (applied/survey), Phase 8 at 10 (synthesis/capstone, naturally the smallest).
...
1. ~~**Phase 1 relabeling** (A1) — split "Computer Basics" into two phases, or leave the label as-is since it already shipped? Leaning toward "leave as-is, note it."~~ **Resolved 2026-08-12, human decision: split, and the Python-heavy half is "Programming Basics," not "Python Fundamentals."** Overrides this doc's own recommendation. Engineering approach: keep `phase_1_computer_basics` as the key for the existing 5 hardware nodes (no renumber), mint a new phase key for the other 13 (e.g. `phase_1b_programming_basics`) rather than renumbering `phase_2` through `phase_8` — a full renumber would ripple through every node's `phase` field, `edges.json`, phase-evaluation file names, `meta.json`, and every phase-scoped UI route, for a labeling change that doesn't need it. Flag back if a true renumber (not a `1b` insert) was actually wanted — this doc is recording the lower-blast-radius interpretation, not assuming it's the only valid one.
2. ~~**Candidate Phase 1-2 additions** (A4/A6) — add before Phase 4-8 authoring, or patch later?~~ **Resolved 2026-08-12, human decision: OOP + calculus + data cleaning, matching this doc's own recommendation.** Exceptions, file I/O, and list comprehensions stay deferred, not added in this pass.
3. **The originally-referenced "~41-node" figure** — resolved: it was an estimate from prior chat discussion, never a saved doc anywhere in this repo. Section B's 72-node breakdown supersedes it; nothing to reconcile against.
4. **Dead-end edges** (A2/A3) — deferred, cosmetic only, confirmed harmless by the validator's existing cycle check.
5. ~~**`topic` as a first-class schema field** (A1.5/Section D)~~ **Resolved 2026-08-12, human decision: yes, add it now**, matching this doc's own recommendation. `topic: string` + `topic_order: number` on `SkillNode` (`src/types/index.ts`) — first task of the follow-up authoring session, before any Phase 4-8 content lands, since every new node should be written directly against the real field rather than an inferred grouping.
6. ~~**Total scope check** — go/no-go on all 119 nodes in one pass vs. phasing it.~~ **Resolved 2026-08-12, human decision: full ~~119~~ **120**-node scope is committed, executed as sequential sets, not one pass.** (The "119" figure itself was an arithmetic error, corrected 2026-08-13 in Section B above once Set 6 authoring actually hit the contradiction — the scope decision, "commit to the full plan across sequential sets," stands unchanged; only the total node count was wrong.) Set breakdown for the follow-up session:
   - **Set 1** — `topic`/`topic_order` schema field (E5) + the three P1-2 prerequisite nodes (E2): `p1b_oop_classes`, `p2_calculus_basics`, `p2_data_cleaning_preprocessing`, plus the Phase 1 split (E1).
   - **Set 2** — Phase 4 (16 nodes, 5 topics).
   - **Set 3** — Phase 5 (16 nodes, 5 topics) — only after Set 1's OOP/calculus nodes exist, since Phase 5 hard-depends on both.
   - **Set 4** — Phase 6 (16 nodes, 4 topics).
   - **Set 5** — Phase 7 (14 nodes, 4 topics).
   - **Set 6** — Phase 8 (10 nodes, 3 topics) + full-curriculum `npm run validate` re-check across all **120** nodes.
   Each set ships its own phase evaluation file (BLOOM-6 gate) and is committed/reviewable independently, matching how BLOOM-4 was built once against p1 then reused for p2/p3 — not a single 120-node commit.
