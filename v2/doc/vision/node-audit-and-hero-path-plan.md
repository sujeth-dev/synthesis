...
6. ~~**Total scope check** — go/no-go on all 119 nodes in one pass vs. phasing it.~~ **Resolved 2026-08-12, human decision: full 119-node scope is committed, executed as sequential sets, not one pass.** Set breakdown for the follow-up session:
   ...
   - **Set 6** — Phase 8 (10 nodes, 3 topics) + full-curriculum `npm run validate` re-check across all 119 nodes.
   Each set ships its own phase evaluation file (BLOOM-6 gate) and is committed/reviewable independently, matching how BLOOM-4 was built once against p1 then reused for p2/p3 — not a single 119-node commit.
