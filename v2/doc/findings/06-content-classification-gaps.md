...
**Redefined by human direction 2026-08-10** (see `MASTER_PLAN.md`'s BLOOM-2..6): (b)-(d) above — a per-question `bloom_level` field, retrofitted onto individual questions — is superseded by a **Phase Evaluation** feature instead. The reasoning: skill-level BKT tracking is already the *micro* evaluation layer this finding worried about being undermined; Bloom's Taxonomy is meant to sit one level up as a *macro* evaluation layer — checking whether a learner can operate at every level of thinking across a phase's main concepts, not labeling individual practice questions that BKT already tracks per-skill. Concretely: one optional per-phase evaluation set (p1/p2/p3), ~2-3 questions per Bloom level, sampled across that phase's skills, attempts recorded through the existing single-skill `insertAttempt()`/BKT pipeline unchanged, results shown as a per-Bloom-level breakdown. (a) — the skill-granularity rule — is unaffected by this redefinition and was resolved separately.
